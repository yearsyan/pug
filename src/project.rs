        ...
        ProjectPackKind::Normal => {
            ...
            let export_dir = if platform_supports_normal_pack_dist(platform_name) {
                match pack
                    ...
                    .flatten()
                {
                    Some(dist) => export_dir.join(dist),
                    None => export_dir,
                }
            } else {
...
fn normalize_pack_dist(path: &Path) -> Result<Option<PathBuf>> {
    ...
    for component in path.components() {
        match component {
            ...
            Component::Prefix(_) | Component::RootDir => {
                bail!("dist must be relative: {}", path.display());
            }
            Component::ParentDir => {
                bail!(
                    "dist must stay inside the export directory: {}",
                    path.display()
                );
            }
        }
